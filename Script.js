...
document.addEventListener("DOMContentLoaded", () => {
    // Seleciona as caixas de texto (steps) e as imagens de fundo
    const steps = document.querySelectorAll(".scrolly-step");
    const visuals = document.querySelectorAll(".scrolly-img");

    // Cria um observador de tela
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            // Se a caixa de texto chegar próxima ao centro da tela
            if (entry.isIntersecting) {
                const targetId = entry.target.getAttribute("data-target");
                
                // 1. Apaga todas as imagens ativas suavemente
                visuals.forEach(v => v.classList.remove("active"));
                
                // 2. Acende a imagem correspondente ao texto atual
        ...
        // Opções de precisão (Dispara quando a caixa cruza o centro virtual da tela)
        root: null,
        rootMargin: "-45% 0px -45% 0px", 
        threshold: 0
    });

    // Pede ao observador para ficar vigiando todos os textos
